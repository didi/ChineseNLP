...
NLPCC 2017 KBQA shared task 要求系统能够从提供事实三元组 (factual triples) 的知识库 (knowledge base) 中检索出答案。知识库包含大约8.7 million 实体 (entity)以及 47.9 million 三元组 (triple).
...
测试集由人工标注生成。对于每一个三元组，标注者写下一个自然语言形式的问题 (natural-language question)，其答案是三元组的对象 (object).测试集中只提供对应的问题/答案对，但是不提供生成问题/答案对的三元组。
...
测试集由人工标注生成。标注者从给定的文本 (document) 中挑选出一个句子，并写出一个自然语言形式的问题，该问题的答案就是标注者选出的句子。
...
* WebQA (百度) 数据集包含 42k 问题和答案。 [link](https://arxiv.org/pdf/1607.06275.pdf)
* DuReader (Baidu) 200k提问来自在线query日志. [link](https://arxiv.org/pdf/1711.05073.pdf)
